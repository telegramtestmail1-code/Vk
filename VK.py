...
# ==================== START IMAGE (set to None to disable) ====================
START_IMAGE = "https://graph.org/file/bba6e6eebf053c124978a-f64dc50bd4f9534f3a.jpg"  # or None
...
def stop_all_cloned_bots():
    stopped = []
    for uid in list(cloned_processes.keys()):
        if stop_cloned_bot(uid):
            stopped.append(uid)
    # Clear persistence
    save_cloned_bots(set())
    return stopped
...
# ==================== SAFE TASK FUNCTIONS (with CPU‑friendly sleeps) ====================
    ...
    try:
        while True:
            ...
            try:
                ...
                pass
            await asyncio.sleep(0.2)  # CPU‑friendly
    except asyncio.CancelledError:
        raise
    except Exception as e:
...
async def handle_dot_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        known_commands = [
            "start", "dclone", "unclone", "leave", "help", "sudo", "rmsudo", "csudo", "rmcsudo",
        ...
        if cmd == "unclone":
            ...
            if args and args[0].lower() == "all":
                await unclone_all(update, context)
            else:
                await message.reply_text("» ᴜsᴀɢᴇ: .unclone all")
            return
...
async def show_start_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, query=None):
    try:
        ...
        if START_IMAGE and (query is None):
            try:
...
async def unclone_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner command to stop all cloned bots and clear persistence."""
    try:
        stopped = stop_all_cloned_bots()
        if stopped:
            await update.effective_message.reply_text(
                f"» ᴀʟʟ ᴄʟᴏɴᴇᴅ ʙᴏᴛs sᴛᴏᴘᴘᴇᴅ ᴀɴᴅ ʀᴇᴍᴏᴠᴇᴅ ✅\n"
                f"» sᴛᴏᴘᴘᴇᴅ {len(stopped)} ʙᴏᴛs: {', '.join(str(uid) for uid in stopped)}"
            )
        else:
            await update.effective_message.reply_text("» ɴᴏ ᴄʟᴏɴᴇᴅ ʙᴏᴛs ᴡᴇʀᴇ ʀᴜɴɴɪɴɢ.")
    except Exception as e:
        logger.error(f"Error in unclone_all: {e}", exc_info=True)
        await update.effective_message.reply_text("» ᴀɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ ᴡʜɪʟᴇ ᴜɴᴄʟᴏɴɪɴɢ.")
...
def main():
    ...
    print("BOT IS WORKING (crash‑proof + CPU‑optimized + all features)")
    try:
